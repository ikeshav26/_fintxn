...
export const addBenificary = async (req, res) => {
  try {
    ...
    if (toBeConnectAccount.user.toString() === user) {
      return res
        .status(400)
        .json({ message: "You cannot add your own account as benificiary" });
    }

    const existingBenificiary = await Benificiary.findOne({
      user,
      connectedAccount,
    });
    ...
    res
      .status(201)
      .json({ message: "Benificiary added successfully", benificiary });
  } catch (err) {
...
export const getBenificaries = async (req, res) => {
  try {
    const user = req.user.id;
    const benificaries = await Benificiary.find({ user }).populate({
...
export const deleteBenificary = async (req, res) => {
  try {
    ...
    if (benificiary.user.toString() !== user) {
      return res
        .status(403)
        .json({ message: "You are not authorized to delete this benificiary" });
    }
