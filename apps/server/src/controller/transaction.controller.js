...
import { sendFreeCreditAddedEmail, sendTransactionSuccesfulMail } from "../services/email.service.js";
...
export const createTransaction=async(req,res)=>{
    try{
        ...
        const fromUserAccount=await Account.findById(fromAccount).populate("user","email username");
        const touserAccount=await Account.findById(toAccount).populate("user","email username");
        ...
        await sendTransactionSuccesfulMail(fromUserAccount.user.email,fromUserAccount.user.username,amount,touserAccount.user.username)
...
export const addTestCredits=async(req,res)=>{
    try{
        ...
        if(!accountId || !amount){
            return res.status(400).json({ error: "Missing required fields: accountId, amount" })
        }

        const account=await Account.findById(accountId).populate("user","email username")
        if(!account){
        ...
        res.status(201).json({ message: "Test credits added successfully", transaction: testTransaction })
    }catch(err){
            res.status(500).json({ error: "An error occurred while adding test credits", details: err.message })
